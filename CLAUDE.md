...
**Deferred loading pattern:** a profile's `load.g` is a single call to the shared wrapper `sys/meltingplot/filament_load.g` (`M98 P"0:/sys/meltingplot/filament_load.g" S<active> R<standby>`), which sets temperatures and arms `global.deferred_filament_load[state.currentTool]`, then exits without physically loading. The centralized `sys/meltingplot/load_filament_sensorless_conditionally.g` (run from the machine-generated `config.g` — the only post-M701 hook) consumes the flag and calls `load_filament_sensorless`; it records the dispatched filament in `global.filament_physical_load_name` BEFORE clearing the flag (watchdog race ordering — keep it that way). `unload.g` is likewise one call to `sys/meltingplot/filament_unload.g`.

**Machine-owned config.g:** `filaments/<name>/config.g` is auto-generated boilerplate (override include + conditionally hook) — never hand-edit it. Per-material parameters (M572/M207/M143/M309/…) live in `filaments/<name>/config-override.g`. `tpost0.g` regenerates `config.g` via `sys/meltingplot/regenerate_filament_config.g` on every tool change with an assigned filament; a missing `config-override.g` is created by renaming the existing `config.g` (M471), which auto-migrates foreign/DWC-created profiles. The helper must only run in blocking-capable contexts (tpost0, macros), never in `daemon.g`.

**Broken-profile watchdog (`daemon.g`):** a DWC-created profile missing the standard lines fails silently (M701 registers the filament, nothing physically loads). Since `fileread()` cannot parse G-code files, detection is behavioral: within 60 s of an OM filament-name change either `global.filament_physical_load_name` must match (load dispatched) or the profile is broken — the daemon then distinguishes load.g vs config.g failure via the stuck deferred flag, warns (M291 S1 + M118, naming the missing line and the `repair-filament-profile` macro), captures the name in `global.filament_broken_profile`, and force-unloads: `global.filament_forced_unload = true` + `M702 P0`. The forced flag makes `filament_unload.g`/`unload_filament` return immediately (first-statement guard), so the daemon's M702 only clears the assignment and cannot block. The name→"" transition without `filament_physical_unload_done` flags a broken `unload.g`. `macros/meltingplot/maintenance/repair-filament-profile` is the canonical fix (regenerates config.g, optionally rewrites load.g/unload.g templates with prompted temps).
...
Calibration macros generate config files in filament directories (e.g., `config-auto-esteps.g`, `config-auto-nle.g`), and `config.g` itself is machine-generated by `regenerate_filament_config.g`. None of these should be hand-edited — per-material customizations belong in `config-override.g`.
