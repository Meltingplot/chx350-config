...
- `daemon.g` validates the flags that persist between iterations: door states, switch_checked flags or `machine_mode` holding neither valid value halt with M112 (nothing but the daemon and the two operating-mode scripts write them); the Z stall deadline outside `[upTime, upTime+30]` halts. Numeric timers are plausibility-checked and read as timed out / re-dated with an `M118` warning (`idle_since` in the future, motion timestamps more than 1 s in the future — the slack absorbs the non-atomic `upTime`+`msUpTime` pair, two separate OM lookups whose sum reads up to 1 s low when a second rolls over between them, `idle_heater_timeout` capped at 7200 s, `z_motor_stall_time_max` outside 1..30 arms 5 s).
- Numeric values cannot be coerced (they need `<`/`>`), so a type error there still aborts the daemon. That is why the daemon's block order is fixed: tracker → doors → mirror → hot → mode → LED contain only coerced comparisons and always run; idle cutoff, Z watchdog, filament and MFM follow. Do not move a numeric comparison in front of the mode block.
...
**`abort` is forbidden in the filament load/unload path.** `abort` terminates all nested macros *and the current print file* — so a broken filament profile, a missing `temps.g` or a stray console `M701`/`M702` would kill a running job over a configuration error. `filament_load.g`, `filament_unload.g` and everything they call warn and return instead: `M118 P0 S"Error: …"` (P0 reaches every channel; `echo` only answers the calling one) followed by `M99`. Returning early is the safe outcome, not a silent one — the load path bails out before arming `deferred_filament_load`, and the unload path before the physical unload, which is exactly the signature the broken-profile watchdog in `daemon.g` detects, reports and cleans up.
...
**Nozzle diameter and nozzle-specific profile data:** a profile describes a *material*, not a material/nozzle combination — there is one `PLA` profile, not `PLA 0.4mm` and `PLA 0.8mm`. The installed nozzle is a property of the **tool**: `global.nozzle_diameter[tool]` is declared in `globals` and persisted per tool in `sys/meltingplot/nozzle<tool>.g`, written by the shared `sys/meltingplot/store_nozzle_diameter.g` (`D<diameter> T<tool>`, hands the two-decimal key back in `global.result`) and loaded by `globals` (fileexists-guarded) — a nozzle change must survive a power cycle. Two callers ask for the value: the `macros/meltingplot/maintenance/set-nozzle-diameter` macro, and `sys/meltingplot/confirm_nozzle_diameter.g`, which `load_filament_sensorless` runs at the feed prompt — after a filament change was triggered but **before** `calibrate_e_steps` — as a dropdown with the configured diameter preselected, so the nozzle-specific calibration files are never written under a stale key. Cancelling it keeps the configured value (`J2`, never `J1` — the confirmation must not abort a load), and a changed diameter is re-applied with `load_nozzle_config.g`, **not** `M703`: that macro runs from inside the profile's own `config.g`, so `M703` would re-enter the executing file. Like `last-filament-temp.g` these files are machine-local state: they are written on the machine, never shipped with the repo, and a config update must not overwrite them.

**Never concatenate the raw diameter into a file name.** RRF keeps the decimal places the value was written with: `0.4` renders `"0.4"`, `0.4000` renders `"0.4000"`, and any computed value renders `"0.4000000"` — three different, silently wrong file names for the same nozzle. The key is always derived as a fixed-width two-decimal string by `sys/meltingplot/sformat.g`: `M98 P"0:/sys/meltingplot/sformat.g" F{global.nozzle_diameter[<tool>]} D2 W0` → `"0.40"`, `"1.20"`. `sformat.g` returns through `global.result` (RRF macros cannot return a value). **`global.result` is only valid immediately after the call that set it** — no macro saves or restores it, and none may assume it survives an intervening call. A macro that needs an error code captures it into a local `var` on the very next line (see `load_filament_sensorless` around the `calibrate_e_steps` call); a macro that produces one sets it explicitly. Everything that depends on the nozzle lives in `filaments/<name>/nozzle-<key>.g` (max volumetric flow rate, M572, M207, M309); the machine-generated `config.g` runs `config-override.g` first and then `sys/meltingplot/load_nozzle_config.g S"<name>"`, which picks `nozzle-<key>.g` of the current tool if it exists, so nozzle values override material values. That selection deliberately sits in a static macro rather than being emitted into `config.g`: keep the generated `config.g` to plain `M98` calls. Meta code (`var`/`if`) written by `echo` needs quotes inside quotes, exists only on the machine and is reviewable nowhere, so a quoting mistake in it is invisible in the repo and breaks all 30 profiles at once — while the same logic in a static macro is ordinary, diffable source. A missing `nozzle-<key>.g` is not an error — the material-wide values from `config-override.g` then stand, which is what keeps profiles predating the split working. It is nevertheless *offered* rather than merely tolerated: `sys/meltingplot/create_nozzle_config.g` asks whether to add the values for the installed nozzle (enter them, write defaults, or not now) and writes the file, handing back `0` in `global.result` when it wrote one so the caller can apply it with `load_nozzle_config.g`. That offer is called only from the interactive paths — `confirm_nozzle_diameter.g` (at the filament-load prompt, where the operator has just selected the profile and the new values still reach the load and the e-steps/NLE calibrations that follow), `set-nozzle-diameter` after a nozzle change, and `create-filament-profile`, which shares the same prompts instead of duplicating them. It must never move into `load_nozzle_config.g` itself: that one runs from the generated `config.g` on every `M703`, hence also from `print_start`, `pause.g`, `resume.g`, `trigger3.g` and the MFM auto-recovery, where a blocking `M291` would stall the print — and the load path already sits at macro nesting depth 7 of the 10 RRF allows. Because M572/M207/M309/flow rate are *global machine state* (whatever the previously loaded filament set stays active), the nozzle file always writes all of them explicitly instead of only the deviating ones.
...
Calibration macros generate config files in filament directories, and `config.g`, `load.g` and `unload.g` are machine-generated by `regenerate_filament_config.g`. None of these should be hand-edited — per-material customizations belong in `config-override.g` (parameters), `nozzle-<key>.g` (nozzle-specific parameters) and `temps.g` (temperatures).

A calibration result only ever describes the nozzle it was measured with, so `calibrate_e_steps`, `calibrate_nle` and `calibrate_pa` write `config-auto-esteps-<key>.g`, `config-auto-nle-<key>.g` and `config-auto-pa-<key>.g` (`<key>` = the two-decimal nozzle key, e.g. `0.40`). `tpost0.g` loads the file matching the installed nozzle and falls back to the unsuffixed name of profiles predating the split; `load_filament_sensorless` applies the same fallback when deciding whether an NLE calibration exists.
