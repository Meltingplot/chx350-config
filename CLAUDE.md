...
- `daemon.g` validates the flags that persist between iterations: door states, switch_checked flags or `machine_mode` holding neither valid value halt with M112 (nothing but the daemon and the two operating-mode scripts write them); the Z stall deadline outside `[upTime, upTime+30]` halts. Numeric timers are plausibility-checked and read as timed out / re-dated with an `M118` warning (`idle_since` in the future, motion timestamps in the future, `idle_heater_timeout` capped at 7200 s, `z_motor_stall_time_max` outside 1..30 arms 5 s).
