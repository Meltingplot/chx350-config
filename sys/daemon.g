...
while state.status != "halted" && global.daemon_reload == false
  ...
  while iterations < #move.axes
    if global.last_machine_position[iterations] != move.axes[iterations].machinePosition
      ...
      set global.last_axis_motion_time[iterations] = state.upTime + state.msUpTime/1000
      set global.potential_unsafe_state = true
    elif (state.upTime + state.msUpTime/1000 - global.last_axis_motion_time[iterations]) < 0.25
      set global.potential_unsafe_state = true
    else
  ...
  while iterations < #move.extruders
    if global.last_extruder_position[iterations] != move.extruders[iterations].position
      ...
      set global.last_extruder_motion_time[iterations] = state.upTime + state.msUpTime/1000
      set global.potential_unsafe_state = true
    elif (state.upTime + state.msUpTime/1000 - global.last_extruder_motion_time[iterations]) < 0.25
      set global.potential_unsafe_state = true
    else
  ...
  if state.status == "processing" && job.file.fileName != null
    if global.mfm_suppress_until > 0 && (state.upTime + state.msUpTime/1000) >= global.mfm_suppress_until
    ...
    if ((state.upTime + state.msUpTime/1000) - global.mfm_last_check_time) >= 0.5 && global.mfm_suppress_until == 0
      set global.mfm_last_check_time = state.upTime + state.msUpTime/1000
        ...
        if global.mfm_last_pct != null
          ...
          if abs(var.pct - global.mfm_last_pct) >= 80
            ...
            if global.mfm_swing_count == 1
              set global.mfm_suppress_until = state.upTime + state.msUpTime/1000 + 15
              ...
              set global.mfm_suppress_until = state.upTime + state.msUpTime/1000 + 60
              set global.ignoreMFMevents = true
              set global.mfm_swing_count = 0
              M220 S100
              set global.mfmbackoff = 3
  ...
  set global.daemon_cycle_time = state.upTime + state.msUpTime/1000 - var.iter_start
  G4 P100
