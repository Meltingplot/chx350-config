...
; print/finish.g's legitimate downgrade accepted in the same step); a global holding neither
...
while state.status != "halted" && global.daemon_reload == false
  ...
  while iterations < #move.axes
    if global.motion_axis_pos[iterations] != move.axes[iterations].machinePosition
      ...
      set global.motion_axis_delta[iterations] = 0
  while iterations < #move.extruders
    if global.motion_extruder_pos[iterations] != move.extruders[iterations].position
  ...
  if ("" ^ global.machine_is_hot) != "2863311530" && ("" ^ global.machine_mode) != "automatic"
    if global.led_color != 4
      M98 P"0:/sys/meltingplot/lib/set-led-color.g" C"red"
  elif state.status == "paused" && ("" ^ global.potential_unsafe_state) == "2863311530"
    M98 P"0:/sys/meltingplot/lib/set-led-color.g" C"pulse_white"
  elif ("" ^ global.machine_mode) == "automatic"
    if job.file.fileName == null
      if global.led_color != 2
        M98 P"0:/sys/meltingplot/lib/set-led-color.g" C"green"
    elif global.led_color != 0
      M98 P"0:/sys/meltingplot/lib/set-led-color.g" C"white"
  elif global.led_color != 1
    M98 P"0:/sys/meltingplot/lib/set-led-color.g" C"blue"
  ...
  if global.filament_load_failed == true && move.extruders[0].filament != ""
    ; the physical unload already ran in filament/load-procedure.g's error paths (all of
    ; them call filament/unload-procedure.g, which sets filament_unload_done) - so just
    ; drop the assignment: M702 P0 clears the loaded-filament name WITHOUT running
    ; unload.g, so it cannot block the daemon
    M702 P0
    set global.filament_load_failed = false
  ...
  ; must have been dispatched (marker set by filament/on-config.g) or
  ...
  ; filament_unload_skip makes unload.g return immediately (and self-clears there), so the
  ...
  if move.extruders[0].filament != var.fil_prev
    if move.extruders[0].filament == ""
      ...
      ; daemon-forced unload sets filament_unload_done itself, so this stays
      ; quiet after the broken-profile force-unload below.
      if global.filament_unload_done == false
        ...
        M118 P0 S{"Filament profile '" ^ var.fil_prev ^ "' broken: unload.g did not run the shared filament/on-unload.g - run macro repair-filament-profile"}
      set global.filament_load_dispatched = ""
  ...
  if var.fil_watch_until != 0
    if global.filament_load_dispatched == var.fil_prev
      set var.fil_watch_until = 0 ; physical load dispatched - failures beyond this point raise filament_load_failed
    elif var.now >= var.fil_watch_until
      set var.fil_watch_until = 0
      set global.filament_broken_profile = var.fil_prev
      if global.filament_load_pending[0]
        ; load.g armed the deferred flag but nothing consumed it -> config.g misses the hook (or M703 was never sent)
        set global.filament_load_pending[0] = false
        ...
        M118 P0 S{"Filament profile '" ^ var.fil_prev ^ "' broken: load.g did not run the shared filament/on-load.g - run macro repair-filament-profile"}
      ...
      ; iteration. filament_unload_skip self-clears inside the M702's unload.g.
  ...
  ; and filament/mfm-recovery.g re-snapshots afterwards. Single OM read, non-blocking.
      ...
      if var.pct != null && var.pct != global.mfm_prev_percentage
        ...
        if global.mfm_error_start_pos != null
          if var.pct > 80 && var.pct < 150
            ...
            set global.mfm_normal_since = 0
        if global.mfm_prev_percentage != null
          if (var.now - global.mfm_swing_window_start) > 300
            set global.mfm_swing_count = 0
            set global.mfm_swing_window_start = var.now
          if abs(var.pct - global.mfm_prev_percentage) >= 80
            if global.debug
              echo "MFM: large swing (" ^ {global.mfm_prev_percentage} ^ "% → " ^ {var.pct} ^ "%)"
