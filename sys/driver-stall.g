...
; sensorless_z_homing and z_motor_stall_count[] are bit-flip hardened (see globals):
...
if param.B == 0 && param.D < 4
  if ("" ^ global.sensorless_z_homing) == "1431655765"
    ...
    set global.z_motor_stall_count[param.D] = 0x55555555
    ...
    if global.z_motor_stall_time == 0
      ...
      if global.z_motor_stall_time_max >= 1 && global.z_motor_stall_time_max <= 30
        ...
        set global.z_motor_stall_time = state.upTime + 5

    if ("" ^ global.z_motor_stall_count[0]) == "1431655765" && ("" ^ global.z_motor_stall_count[1]) == "1431655765" && ("" ^ global.z_motor_stall_count[2]) == "1431655765" && ("" ^ global.z_motor_stall_count[3]) == "1431655765"
      set global.z_motor_stall_time = 0 ; reset timer
      set global.z_motor_stall_count = vector(4, 0xAAAAAAAA) ; reset counter
  else
