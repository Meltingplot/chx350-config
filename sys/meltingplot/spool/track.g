...
; bracketed by the firmware hooks: print/prepare.g (start.g, once per job start) arms
...
; at all, which is what lets print/finish.g run any number of times after a job (end-gcode,
...
;   M98 P"0:/sys/meltingplot/spool/track.g"      - book only (daemon.g, every 60 s)
;   M98 P"0:/sys/meltingplot/spool/track.g" W1   - book, persist (spool/store.g), report
; Without W1 this is OM reads and set only - non-blocking, no file write - which is what
; lets daemon.g call it. W1 writes files and belongs to print/finish.g.

if global.debug
  echo "spool/track.g"

; not armed: no job is being tracked (never started through print/prepare.g, or already
; flushed) - nothing to book, and a repeated print/finish.g must not book the job twice
...
while iterations < min(#tools, 2)
  if tools[iterations] != null
    ...
    if var.raw != global.spool_track_baseline[iterations]
      if global.spool_density[iterations] > 0
        ; signed: a sample taken mid-retraction books a few mm back, the next one re-books them
        var mm = {(var.raw - global.spool_track_baseline[iterations]) * move.extruders[var.e].factor}
        ...
        if global.spool_net_weight[iterations] > 0 && global.spool_remaining[iterations] > global.spool_net_weight[iterations]
          set global.spool_remaining[iterations] = global.spool_net_weight[iterations]   ; a booked-back retraction cannot overfill the spool
      set global.spool_track_baseline[iterations] = var.raw
    if var.persist
      M98 P"0:/sys/meltingplot/spool/store.g" T{iterations}
      set global.spool_track_baseline[iterations] = 0.0        ; print over - RRF zeroes rawPosition at the next start
      if global.spool_density[iterations] > 0
...
; flushed - disarm, so that every further print/finish.g of this job books nothing
if var.persist
