...
;   M98 P"0:/sys/meltingplot/filament/on-load.g" F"<profile name>"
...
if exists(param.F)
  ...
  if global.filament_temp_active <= 0
    M118 P0 S{"Error: filament/on-load.g: filaments/" ^ param.F ^ "/temps.g missing or invalid - run macro repair-filament-profile"}
    M99
  ...
  M118 P0 S"Error: filament/on-load.g: missing F (profile name) or S (active temperature) parameter"
  M99

if state.currentTool == -1
  T0 ; console M701 without a tool selected - use the default tool
if state.currentTool == -1
  M118 P0 S"Error: filament/on-load.g: no tool selected"
  M99
...
set global.filament_load_pending[state.currentTool] = true
