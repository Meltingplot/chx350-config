...
; without filament_unload_done is what the daemon watchdog reports.
...
if exists(param.F)
  ...
  if global.filament_temp_active <= 0 && global.filament_temp_unload <= 0
    M118 P0 S{"Error: filament/on-unload.g: filaments/" ^ param.F ^ "/temps.g missing or invalid - run macro repair-filament-profile"}
    M99
  ...
  M118 P0 S"Error: filament/on-unload.g: missing F (profile name) or S (active temperature) parameter"
  M99

if state.currentTool == -1
  T0 ; console M702 without a tool selected - use the default tool
if state.currentTool == -1
  M118 P0 S"Error: filament/on-unload.g: no tool selected"
  M99
