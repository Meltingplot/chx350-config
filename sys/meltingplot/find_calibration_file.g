...
;   1. filaments/<S>/<F>-<filament>-<nozzle>.g   the current key (calibration_key.g)
...
;   M98 P"0:/sys/meltingplot/find_calibration_file.g" S"<filament name>" F"config-auto-nle" T0
...
; call). Readers: tpost0.g (loads the three calibration results), load_nozzle_config.g,
; load_filament_sensorless (is an NLE calibration present?), create_nozzle_config.g and
; macro create-filament-profile (does the profile already describe this hardware?).
; The key format lives in calibration_key.g; it is repeated here because the fallback
; needs the nozzle half on its own and RRF has no substring operator.

if global.debug
  echo "find_calibration_file.g"

if !exists(param.S) || !exists(param.F) || param.S == "" || param.F == ""
  M118 P0 S"Error: find_calibration_file.g: missing S (filament) or F (base name) parameter"
  set global.result = ""
  M99
