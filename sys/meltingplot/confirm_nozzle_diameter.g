...
if var.diameter == global.nozzle_diameter[var.tool]
  M99                      ; confirmed unchanged - nothing to persist and nothing to re-apply
...
; for the load and the e-steps calibration that follow
...
if var.filament != ""
  ...
  if !fileexists("0:/filaments/" ^ var.filament ^ "/nozzle-" ^ var.key ^ ".g")
    M118 P0 S{"Warning: '" ^ var.filament ^ "' has no values for " ^ var.key ^ "mm - run macro create-filament-profile"}
