; filament-profile/create-nozzle-file.g
...
; from filament-profile/calibration-key.g - and prompts for its values (max volumetric flow rate,
; pressure advance, firmware retraction, heater feedforward).
;   M98 P"0:/sys/meltingplot/filament-profile/create-nozzle-file.g" S"<filament name>" T0
...
; callers differ: hardware/confirm-nozzle-diameter.g and macro set-nozzle-diameter run
; filament-profile/apply-nozzle-file.g afterwards, while macro create-filament-profile writes for a
...
; filaments/<name>/config.g, which also runs from print/prepare.g, pause.g and resume.g,
; where a prompt would stall the print. That is why the offer sits in the interactive
; callers and not in filament-profile/apply-nozzle-file.g, which they all pass through.
...
if global.debug
  echo "filament-profile/create-nozzle-file.g"

if !exists(param.S)
  M118 P0 S"Error: filament-profile/create-nozzle-file.g: missing S (filament profile name) parameter"
  set global.result = 1
  M99
if param.S == ""
  set global.result = 1
...
; the same fallback chain filament-profile/apply-nozzle-file.g uses, so a nozzle-only file from before
...
M98 P"0:/sys/meltingplot/filament-profile/find-calibration-file.g" S{param.S} F"nozzle" T{var.tool}
if global.result != ""
  set global.result = 1
  M99

; the key is "<filament>-<nozzle>" (filament-profile/calibration-key.g, returns in global.result - read it
